...
Sub deyimler_fabe()
    ...
    Do
        ...
        Loop While (Selection.Font.bold = True Or _
                ...
                count < 5 Or _
                Mid(Selection.text, Len(Selection.text), 1) = "v" Or _
                Mid(Selection.text, Len(Selection.text), 1) = "e" Or _
                Mid(Selection.text, Len(Selection.text), 1) = "y" Or _
                Mid(Selection.text, Len(Selection.text), 1) = "a"
...
Function latcmp(ByVal first As String, ByVal second As String) As Boolean
    Dim i As Variant
    
    'MsgBox first + "--" + second
    i = StrComp(first, second, vbTextCompare) 'vbBinaryCompare)
    If i = -1 Or i = 0 Then
        latcmp = True
    Else
        latcmp = False
    End If
    'MsgBox first + "  " + second + "  " + tostr(i)
End Function
